  ...
  function renderRow(row: SenhaRow, depth = 0) {
    ...
    return (
      <div key={row.id}>
        <div
          className={cn(
            'flex items-center gap-3 px-4 py-3 border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 group transition-colors',
            depth > 0 && 'pl-10 bg-gray-50/50 dark:bg-gray-900/30'
          )}
        >
          {hasChildren ? (
          ...
          <div className="flex-1 min-w-0">
            <p className="font-semibold text-sm text-gray-900 dark:text-white">{row.service}</p>
          ...
          {/* Login */}
          <div className="text-sm text-gray-600 dark:text-gray-300 min-w-0 w-36 truncate hidden sm:block">{row.login}</div>
          ...
          {/* Notes */}
          {row.notes && <p className="text-xs text-gray-400 dark:text-gray-500 max-w-[120px] truncate hidden lg:block">{row.notes}</p>}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-bold text-gray-900 dark:text-white">Senhas</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Credenciais de acesso — dados armazenados apenas no repositório privado</p>
        </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden">
        {/* Table header */}
        <div className="flex items-center gap-3 px-4 py-2.5 bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-700 text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide">
          <div className="w-4" />
          <div className="w-6" />
        ...
        {senhas.length === 0 ? (
          <div className="py-16 text-center">
            <KeyRound className="w-10 h-10 text-gray-200 dark:text-gray-700 mx-auto mb-3" />
            <p className="text-gray-400 dark:text-gray-500">Nenhuma credencial cadastrada</p>
          </div>
        ) : (
